...
The following Windows types shall be supported out of the box, aliased to their underlying types:
...
The last modifier in a chain overrides previous modifiers if any collision occurs. Example: A ValidateArgument after a previous IgnoreArgument will still validate the argument.
...
The lock factory consists of the following function:

```c
typedef UMOCK_C_LOCK_HANDLE(*UMOCK_C_LOCK_FACTORY_CREATE_LOCK_FUNC)(void* params);
```

Whenever `umock_c` needs to instantiate a lock that is to be used for multithreading access, the factory function shall be called to create one.

A `umock_c` lock implements an interface that comprises of functions for:

- acquiring and releasing the lock in exclusive mode,

- acquiring and releasing the lock in shared mode
...
Whenever an expected call is stored or accessed, if `umock_c` was initialized with a lock factory, `umock_c` shall protect the access to the expected calls structure with the lock type as write.

Whenever an actual call is stored or accessed, if `umock_c` was initialized with a lock factory, `umock_c` shall protect the access to the actual calls structure.

Note: the expected call modifiers are currently not protected, for now the user code is responsible to synchronize expected call modifiers (all expected calls with modifiers have to be serialized by the test author).
